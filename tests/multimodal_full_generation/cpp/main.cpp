...
bool hidden_passed(const Metrics& metrics, bool use_packing_layout)
{
    // The packed vision merger's accepted FP32 error propagates through
    // prefill; exact tokens, logits parity, and KV-prefix immutability remain
    // independently enforced below.
    const double maximum = use_packing_layout ? 5.0e-1 : 5.0e-3;
    const double mean = use_packing_layout ? 1.0e-2 : 5.0e-5;
    const double cosine = use_packing_layout ? 0.999 : 0.999999;
    return metrics.maximum_abs_error <= maximum
        && metrics.mean_abs_error <= mean
        && metrics.cosine_similarity >= cosine;
}

bool cache_passed(const Metrics& metrics, bool use_packing_layout)
{
    const double maximum = use_packing_layout ? 5.0e-1 : 2.0e-3;
    const double mean = use_packing_layout ? 1.0e-2 : 2.0e-5;
    const double cosine = use_packing_layout ? 0.999 : 0.999999;
    return metrics.maximum_abs_error <= maximum
        && metrics.mean_abs_error <= mean
        && metrics.cosine_similarity >= cosine;
...
bool prefill_hidden_passed(const Metrics& metrics, bool use_packing_layout)
{
    return hidden_passed(metrics, use_packing_layout);
}

bool prefill_cache_passed(const Metrics& metrics, bool use_packing_layout)
{
    return cache_passed(metrics, use_packing_layout);
}
...
int main(int argc, char** argv)
{
    ...
    const double tail_maximum = use_packing_layout ? 5.0e-2 : 1.0e-4;
    const double tail_mean = use_packing_layout ? 5.0e-3 : 2.0e-5;
    const double tail_cosine = use_packing_layout ? 0.9999 : 0.999999;

    std::cout << "===== HunyuanOCR ncnn image-to-text runtime =====\n"
              << "packing layout : " << std::boolalpha
              << use_packing_layout << '\n'
              << "input image    : tests/assets/ocr_smoke_en.png\n"
              << "vision models : patch + 27 blocks + merger\n"
    ...
    if (!build_multimodal_prefill_input(
            project_root,
            project_root + "/tests/assets/ocr_smoke_en.png",
            use_packing_layout,
            embedding_network,
            multimodal_input)) {
        std::cerr << "Failed to build multimodal prefill input\n";
        return EXIT_FAILURE;
    }
    std::vector<float> prefill_input_values =
        std::move(multimodal_input.hidden_states);
    std::cout << "===== C++ multimodal prefill input =====\n"
              << "original image : " << multimodal_input.original_width
              << 'x' << multimodal_input.original_height << '\n'
              << "resized image  : " << multimodal_input.resized_width
              << 'x' << multimodal_input.resized_height << '\n'
              << "image grid THW : " << multimodal_input.image_grid_thw[0]
              << 'x' << multimodal_input.image_grid_thw[1]
              ...
              << '\n'
              << "resized RGB max/mean : "
              << multimodal_input.resized_rgb_metrics.maximum_abs_error
              << " / "
              << multimodal_input.resized_rgb_metrics.mean_abs_error << '\n'
              << "pixel values max/mean: "
              << multimodal_input.pixel_values_metrics.maximum_abs_error
              << " / "
              << multimodal_input.pixel_values_metrics.mean_abs_error << '\n'
              << "vision output max/mean: "
              << multimodal_input.vision_embedding_metrics.maximum_abs_error
              << " / "
              << multimodal_input.vision_embedding_metrics.mean_abs_error << '\n'
              << "text embedding max   : "
              << multimodal_input.text_embedding_metrics.maximum_abs_error
              << '\n'
              << "fused hidden max/mean: "
              << multimodal_input.fused_hidden_metrics.maximum_abs_error
              << " / "
    ...
    for (int layer = 0; layer < kLayerCount; ++layer) {
        ...
        if (!compare_mat(prefill_hidden, expected_hidden, input_metrics)
            || !prefill_hidden_passed(
                input_metrics, use_packing_layout)) {
        ...
        if (!compare_mat(next_hidden, expected_output, output_metrics)
            ...
            || !prefill_hidden_passed(
                output_metrics, use_packing_layout)
            || !prefill_cache_passed(key_metrics, use_packing_layout)
            || !prefill_cache_passed(value_metrics, use_packing_layout)) {
            std::cerr << "Prefill layer " << layer
    ...
    if (!compare_mat(
        ...
        || !prefill_hidden_passed(
            prefill_norm_input_metrics, use_packing_layout)) {
    ...
    if (!compare_mat(
        ...
        || !prefill_hidden_passed(
            prefill_norm_metrics, use_packing_layout)
        || prefill_logits_metrics.maximum_abs_error > tail_maximum
        || prefill_logits_metrics.mean_abs_error > tail_mean
        || prefill_logits_metrics.cosine_similarity < tail_cosine) {
    ...
    for (int step = 1; step <= kDecodeSteps; ++step) {
        ...
        for (int layer = 0; layer < kLayerCount; ++layer) {
            ...
            if (!compare_mat(current_hidden, expected_hidden, input_metrics)
                || !hidden_passed(input_metrics, use_packing_layout)) {
            ...
            const bool key_handoff_passed = step == 1
                ? prefill_cache_passed(
                    past_key_metrics, use_packing_layout)
                : cache_passed(past_key_metrics, use_packing_layout);
            const bool value_handoff_passed = step == 1
                ? prefill_cache_passed(
                    past_value_metrics, use_packing_layout)
                : cache_passed(past_value_metrics, use_packing_layout);
            ...
            if (!compare_mat(next_hidden, expected_output, output_metrics)
                ...
                || !hidden_passed(output_metrics, use_packing_layout)
                || !cache_passed(key_metrics, use_packing_layout)
                || !cache_passed(value_metrics, use_packing_layout)) {
                std::cerr << "Step " << step << " layer " << layer
                          << " output parity failed: hidden="
                          << output_metrics.maximum_abs_error
                          << ", key=" << key_metrics.maximum_abs_error
        ...
        if (!compare_mat(
            ...
            || !hidden_passed(
                norm_input_metrics, use_packing_layout)) {
        ...
        if (!compare_mat(norm_output, expected_norm_output, norm_metrics)
            ...
            || norm_metrics.maximum_abs_error > tail_maximum
            || norm_metrics.mean_abs_error > tail_mean
            || logits_metrics.maximum_abs_error > tail_maximum
            || logits_metrics.mean_abs_error > tail_mean
            || logits_metrics.cosine_similarity < tail_cosine) {
            std::cerr << "Step " << step
