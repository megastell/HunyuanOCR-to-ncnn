    ...
    char** argv)
{
    ...
    constexpr int kStep3MaskLength = 316;
    constexpr int kStep3PastLength = 315;
    constexpr int kStep3PresentLength = 316;
    ...
    const std::size_t step3_mask_count =
        static_cast<std::size_t>(
            kStep3MaskLength
        );

    const std::size_t step3_past_count =
        static_cast<std::size_t>(
            kKvHeads
        )
        * static_cast<std::size_t>(
    ...
    const std::size_t step3_present_count =
        static_cast<std::size_t>(
            kKvHeads
        )
        * static_cast<std::size_t>(
    ...
    if (
        ...
        std::cerr
            << "Step 1 present and Step 2 past counts differ.\n";
    ...
    if (
        ...
        std::cerr
            << "Step 2 present and Step 3 past counts differ.\n";
    ...
    const std::string step3_layer0_reference =
        project_root
        + "/artifacts/decoder_layer0_decode_step3/reference";
    ...
    std::vector<float> step3_mask_values;
    std::vector<float> step3_rope_cos_values;
    std::vector<float> step3_rope_sin_values;
    ...
    if (
        ...
        || !load_exact_binary(
            step3_layer0_reference
                + "/layer0_position_embeddings_0_f32.bin",
            rope_count,
            step3_rope_cos_values
        )
        || !load_exact_binary(
            step3_layer0_reference
                + "/layer0_position_embeddings_1_f32.bin",
            rope_count,
            step3_rope_sin_values
        )
    ) {
        std::cerr
            << "共享输入张量加载失败。\n";
    ...
    ncnn::Mat step3_rope_cos =
        make_rope(
            step3_rope_cos_values
        );

    ncnn::Mat step3_rope_sin =
        make_rope(
            step3_rope_sin_values
        );
    ...
    if (
        ...
        || step3_mask.empty()
        || step3_rope_cos.empty()
        || step3_rope_sin.empty()
    ) {
        std::cerr
            << "共享ncnn::Mat创建失败。\n";
    ...
    double step3_maximum_boundary_error = 0.0;
    double step3_maximum_output_error = 0.0;
    double step3_maximum_key_error = 0.0;
    double step3_maximum_value_error = 0.0;

    double maximum_handoff_key_error = 0.0;
    double maximum_handoff_value_error = 0.0;

    double maximum_step3_handoff_key_error = 0.0;
    double maximum_step3_handoff_value_error = 0.0;
    ...
    double maximum_step3_key_prefix_error = 0.0;
    double maximum_step3_value_prefix_error = 0.0;

    double maximum_step3_appended_key_error = 0.0;
    double maximum_step3_appended_value_error = 0.0;

    std::cout
        << "===== Decoder 24-layer three-token runtime =====\n"
        << "threads                    : "
        << kRuntimeThreads
        << '\n'
        << "packing layout             : "
        ...
        << "Step 3 hidden reload       : disabled\n"
        << "Step 2 initial hidden      : Step1 token -> ncnn Embed\n"
        << "Step 3 initial hidden      : Step2 token -> ncnn Embed\n"
        << "Step 2 KV inference reload : disabled\n"
        << "Step 3 KV inference reload : disabled\n"
        << "hidden handoff             : "
    ...
    for (
...

        {
            ...
            step2_present_keys[layer] =
                present_key.clone();

            step2_present_values[layer] =
                present_value.clone();
        }
        ...
        if (
            ...
            || present_key.empty()
            || present_value.empty()
            || step2_present_keys[layer].empty()
            || step2_present_values[layer].empty()
        ) {
            std::cerr
                << "Step 2 Layer "
                << layer
     ...
     *     -> Step 3 Layer 0 in0
     */
    constexpr std::size_t kStep2FeedbackVocabSize =
        120818;

    constexpr int kExpectedStep2Token =
        206;

    int step2_actual_token =
        -1;

    ncnn::Mat step3_current_hidden;

    {
        ...
        std::vector<float> expected_step2_norm_input;
        std::vector<float> expected_step2_norm_output;
        std::vector<float> expected_step2_logits;
        std::vector<float> expected_step3_initial_hidden;
        ...
        if (
            !load_exact_binary(
                step2_tail_reference
                    + "/final_norm_input_f32.bin",
                hidden_count,
                expected_step2_norm_input
            )
            || !load_exact_binary(
                step2_tail_reference
                    + "/final_norm_output_f32.bin",
                hidden_count,
                expected_step2_norm_output
            )
            || !load_exact_binary(
                step2_tail_reference
                    + "/decode_logits_f32.bin",
                kStep2FeedbackVocabSize,
                expected_step2_logits
            )
            || !load_exact_binary(
                step3_layer0_reference
                    + "/layer0_hidden_states_f32.bin",
                hidden_count,
                expected_step3_initial_hidden
            )
        ) {
            std::cerr
                << "Step 2 feedback reference load failed.\n";

            return EXIT_FAILURE;
        }

        std::vector<float> actual_step2_decoder_output;

        if (
            !unpack_mat(
                step2_current_hidden,
                actual_step2_decoder_output
            )
        ) {
            return EXIT_FAILURE;
        }

        Metrics step2_norm_input_metrics;

        if (
            !calculate_metrics(
                actual_step2_decoder_output,
                expected_step2_norm_input,
                step2_norm_input_metrics
            )
        ) {
            return EXIT_FAILURE;
        }
        ...
        if (
            ...
            std::cerr
                << "Step 2 decoder final hidden exceeded tolerance.\n";

            return EXIT_FAILURE;
        }

        ncnn::Mat step2_norm_handoff;
        Metrics step2_norm_metrics;

        {
            ...
            if (
                ...
                std::cerr
                    << "Step 2 Final RMSNorm model load failed.\n";
            ...
            if (
                ...
                std::cerr
                    << "Step 2 Final RMSNorm input bind failed.\n";
            ...
            if (
                ...
                std::cerr
                    << "Step 2 Final RMSNorm output extract failed.\n";
        ...
        if (step2_norm_handoff.empty()) {
            std::cerr
                << "Step 2 Final RMSNorm out0.clone() failed.\n";
        ...
        const bool step2_norm_passed =
            step2_norm_metrics.maximum_abs_error
                <= 5.0e-5
            && step2_norm_metrics.mean_abs_error
                <= 5.0e-6
            && step2_norm_metrics.cosine_similarity
                >= 0.99999999;

        if (!step2_norm_passed) {
            std::cerr
                << "Step 2 Final RMSNorm exceeded tolerance.\n";

            return EXIT_FAILURE;
        }

        std::vector<float> actual_step2_logits;
        Metrics step2_logits_metrics;

        {
            ...
            if (
                ...
                std::cerr
                    << "Step 2 LM Head model load failed.\n";
            ...
            if (
                ...
                std::cerr
                    << "Step 2 LM Head input bind failed.\n";
            ...
            if (
                ...
                std::cerr
                    << "Step 2 LM Head output extract failed.\n";
        ...
        if (
            actual_step2_logits.size()
            != kStep2FeedbackVocabSize
        ) {
            std::cerr
                << "Step 2 logits count mismatch: "
                << actual_step2_logits.size()
                << '\n';
        ...
        const auto expected_iterator =
            std::max_element(
                expected_step2_logits.begin(),
                expected_step2_logits.end()
            );

        const auto actual_iterator =
            std::max_element(
                actual_step2_logits.begin(),
                actual_step2_logits.end()
            );

        const int step2_expected_token =
            static_cast<int>(
                std::distance(
                    expected_step2_logits.begin(),
                    expected_iterator
        ...
        std::cout
            << "\n===== Step 2 feedback logits parity =====\n";

        print_metrics(
            "Step 2 Decoder -> Final RMSNorm -> LM Head logits",
            step2_logits_metrics
        );

        std::cout
            << "Expected Step 2 token : "
            << step2_expected_token
            << '\n'
            << "Actual Step 2 token   : "
            << step2_actual_token
            << '\n'
        ...
        const bool step2_logits_passed =
            step2_logits_metrics.maximum_abs_error
                <= 3.0e-3
            && step2_logits_metrics.mean_abs_error
                <= 5.0e-5
            && step2_logits_metrics.cosine_similarity
                >= 0.999999
            && step2_expected_token
                == kExpectedStep2Token
            && step2_actual_token
        ...
        if (!step2_logits_passed) {
            std::cerr
                << "Step 2 feedback logits or token check failed.\n";
        ...
        if (token_input.empty()) {
            std::cerr
                << "Step 2 feedback token Mat creation failed.\n";
...

        {
            ...
            if (
                ...
                std::cerr
                    << "Step 2 feedback Token Embedding model load failed.\n";
            ...
            if (
                ...
                std::cerr
                    << "Step 2 feedback Token Embedding input bind failed.\n";
            ...
            if (
                ...
                std::cerr
                    << "Step 2 feedback Token Embedding output extract failed.\n";

                return EXIT_FAILURE;
            }

            step3_current_hidden =
                output.clone();
        }

        if (step3_current_hidden.empty()) {
            std::cerr
                << "Step 2 feedback embedding out0.clone() failed.\n";

            return EXIT_FAILURE;
        }

        std::vector<float> generated_step3_initial_hidden;

        if (
            !unpack_mat(
                step3_current_hidden,
                generated_step3_initial_hidden
            )
        ) {
            return EXIT_FAILURE;
        }

        Metrics step2_feedback_embedding_metrics;

        if (
            !calculate_metrics(
                generated_step3_initial_hidden,
                expected_step3_initial_hidden,
                step2_feedback_embedding_metrics
            )
        ) {
            return EXIT_FAILURE;
        }

        const bool step2_feedback_byte_identical =
            generated_step3_initial_hidden.size()
                == expected_step3_initial_hidden.size()
            && std::equal(
                generated_step3_initial_hidden.begin(),
                generated_step3_initial_hidden.end(),
                expected_step3_initial_hidden.begin()
            );

        std::cout
            << "\n===== Step 2 Token Embedding feedback =====\n";

        print_metrics(
            "Step 2 token 206 -> ncnn Embed -> Step 3 Layer 0 hidden",
            step2_feedback_embedding_metrics
        );
        ...
        if (
            step2_feedback_embedding_metrics.maximum_abs_error
                != 0.0
            || step2_feedback_embedding_metrics.mean_abs_error
                != 0.0
            || !step2_feedback_byte_identical
        ) {
            std::cerr
                << "Step 2 autoregressive embedding feedback was not byte-identical.\n";
    ...
    std::cout
        << "\n===== Step 3: direct same-layer KV chain =====\n"
        << "Step 3 initial hidden source: "
        << "Step 2 actual token -> ncnn Embed\n"
        << "Step 3 initial hidden reload: disabled\n";
    ...
    for (
        ...
        step3_maximum_boundary_error =
            std::max(
                step3_maximum_boundary_error,
                input_metrics.maximum_abs_error
            );

        step3_maximum_output_error =
            std::max(
                step3_maximum_output_error,
                output_metrics.maximum_abs_error
            );

        step3_maximum_key_error =
            std::max(
                step3_maximum_key_error,
                key_metrics.maximum_abs_error
            );

        step3_maximum_value_error =
            std::max(
                step3_maximum_value_error,
                value_metrics.maximum_abs_error
            );

        maximum_step3_handoff_key_error =
            std::max(
                maximum_step3_handoff_key_error,
                handoff_key_metrics.maximum_abs_error
            );

        maximum_step3_handoff_value_error =
            std::max(
                maximum_step3_handoff_value_error,
                handoff_value_metrics.maximum_abs_error
            );

        maximum_step3_key_prefix_error =
            std::max(
                maximum_step3_key_prefix_error,
                key_prefix_error
            );

        maximum_step3_value_prefix_error =
            std::max(
                maximum_step3_value_prefix_error,
                value_prefix_error
            );

        maximum_step3_appended_key_error =
            std::max(
                maximum_step3_appended_key_error,
                appended_key_error
            );

        maximum_step3_appended_value_error =
            std::max(
                maximum_step3_appended_value_error,
                appended_value_error
            );
    ...
    std::cout
        ...
        << std::scientific
        << std::setprecision(10)
        << "===== Three-token chain maximum errors =====\n"
        ...
        << step2_maximum_value_error
        << '\n'
        << "Maximum Step 2 handoff key    : "
        << maximum_handoff_key_error
        << '\n'
        << "Maximum Step 2 handoff value  : "
        << maximum_handoff_value_error
        << '\n'
        ...
        << maximum_step2_appended_value_error
        << '\n'
        << "Step 3 maximum input boundary : "
        << step3_maximum_boundary_error
        << '\n'
        << "Step 3 maximum layer output   : "
        << step3_maximum_output_error
        << '\n'
        << "Step 3 maximum present key    : "
        << step3_maximum_key_error
        << '\n'
        << "Step 3 maximum present value  : "
        << step3_maximum_value_error
        << '\n'
        << "Maximum Step 3 handoff key    : "
        << maximum_step3_handoff_key_error
        << '\n'
        << "Maximum Step 3 handoff value  : "
        << maximum_step3_handoff_value_error
        << '\n'
        << "Maximum Step 3 key prefix     : "
        << maximum_step3_key_prefix_error
        << '\n'
        << "Maximum Step 3 value prefix   : "
        << maximum_step3_value_prefix_error
        << '\n'
        << "Maximum Step 3 appended key   : "
        << maximum_step3_appended_key_error
        << '\n'
        << "Maximum Step 3 appended value : "
        << maximum_step3_appended_value_error
        << '\n';
    ...
    if (
        ...
        std::cerr
            << "\nThree-token chain KV history prefix changed.\n";

        return EXIT_FAILURE;
    }

    /*
     * Step 3 Layer 23 out0
     *     -> Final RMSNorm
     *     -> LM Head
     *     -> logits
     */
    ...
    constexpr int kExpectedStep3Token =
        1717;

    const std::string step3_tail_reference =
        project_root
        + "/artifacts/decode_tail_step3/reference";
    ...
    if (
        ...
        std::cerr
            << "Step 3 tail reference load failed.\n";
    ...
    std::cout
        << "\n===== Step 3 Decoder -> Final RMSNorm boundary =====\n";

    print_metrics(
        "Step 3 Layer 23 chained output vs Final RMSNorm reference input",
        final_norm_input_metrics
    );
    ...
    if (
        ...
        std::cerr
            << "Step 3 decoder final hidden exceeded tolerance.\n";
...

    {
        ...
        if (
            ...
            std::cerr
                << "Final RMSNorm model load failed.\n";
        ...
        if (
            ...
            std::cerr
                << "Final RMSNorm input bind failed.\n";
        ...
        if (
            ...
            std::cerr
                << "Final RMSNorm output extract failed.\n";
    ...
    if (norm_handoff.empty()) {
        std::cerr
            << "Final RMSNorm out0.clone() failed.\n";

        return EXIT_FAILURE;
    }

    std::cout
        << "\n===== Step 3 Final RMSNorm parity =====\n";

    print_metrics(
        "Step 3 Final RMSNorm output",
        norm_metrics
    );
    ...
    if (!norm_passed) {
        std::cerr
            << "Step 3 Final RMSNorm exceeded tolerance.\n";
    ...
    std::cout
        << "\n===== Step 3 full logits parity =====\n";

    print_metrics(
        "Step 3 Decoder -> Final RMSNorm -> LM Head logits",
        logits_metrics
    );

    std::cout
        << "Expected Step 3 token : "
        << expected_token
        << '\n'
        << "Actual Step 3 token   : "
        << actual_token
    ...
    if (!logits_passed) {
        std::cerr
            << "Step 3 full logits check failed.\n";

        return EXIT_FAILURE;
    }

    std::cout
        << "\n===== Three-token logits-chain result =====\n"
        << "Step 1 decoder layers : 0 -> 23\n"
        << "Step 2 decoder layers : 0 -> 23\n"
        << "Step 3 decoder layers : 0 -> 23\n"
        << "Same-layer KV handoff : enabled\n"
        << "Step 1 feedback token : "
        << step1_actual_token
        << '\n'
        << "Step 2 feedback token : "
        << step2_actual_token
        << '\n'
        << "Token Embedding       : executed twice\n"
        << "Step 2 hidden reload  : disabled\n"
        << "Step 3 hidden reload  : disabled\n"
    ...
    std::cout
        << "\nThree-token ncnn FP32 autoregressive chain passed: "
        << step1_actual_token
        << " -> "
        << step2_actual_token
        << " -> "
