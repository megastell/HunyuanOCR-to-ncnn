...
LAYER23_REFERENCE = (
    PROJECT_DIR
    / "artifacts/decoder_layer23_decode_step3/reference"
)
...
REPORT_PATH = (
    PROJECT_DIR
    / "docs/decode_step3_tail_reference.json"
)
...
def main() -> None:
    ...
    if expected_token != EXPECTED_TOKEN:
        raise RuntimeError(
            "已有Step 3参考token不是206。"
        )

    if actual_token != EXPECTED_TOKEN:
        raise RuntimeError(
            "重新计算的Step 3 token不是206。"
        )
    ...
    print(
        "✅ Step 3 Final RMSNorm与LM Head参考捕获成功。"
    )
