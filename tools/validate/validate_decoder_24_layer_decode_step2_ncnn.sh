...
BIN="$ROOT/tests/decoder_layer0_decode_step2/build-ropefix-rmsnorm/decoder_layer0_decode_step2_parity"

SUMMARY="$ROOT/docs/decoder_24_layer_decode_step2_ncnn_summary.txt"
...
do
  NAME="decoder_layer${layer}_decode_step2"
  ...
  if [ -f "$LOG" ]; then
    sed -i       "s/Decoder Layer 0 Decode Step 2/Decoder Layer ${layer} Decode Step 2/g"       "$LOG"
  fi
...
if [ "$FAIL" -ne 0 ]; then
  echo "❌ 至少一层Step 2 ncnn数值验证失败"
  exit 1
fi

echo "✅ 全部24层Step 2 packed ncnn独立数值验证成功"
